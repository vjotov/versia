...
function createWorkItem($params){
	$result = cd_workitem::create_workitem($params['release_id'], $params['vo_id'], $params['wi_name']);
	$responce = array();
...
function updateWorkItem($params) {
	$result = cd_workitem::update_workitem($params['wi_id'], $params['wi_name'],  $params['release_id']);
	$responce = array();
	$responce['result'] = array();
  $responce['error'] = $result;
	return $responce;
...
function getNotAttachedWorkItemList($params) {
	$result = cd_workitem::get_not_attached_workitem_list($params['ws_id'], $params['release_id']);
	$responce = array();
	$responce['result'] = $result['workitem_list'];
  $responce['error'] = $result['error'];
	return $responce;
...
class j_workitem { 
	function registerFncs() {
		...
		$functions[] = array('method' => 'createWorkItem', 
			'params' => array('release_id' => 'number', 'vo_id' => 'number', 'wi_name' => 'string'), 
		...
		$functions[] = array('method' => 'getWorkItemList', 
			'params' => array('ws_id' => 'number', 'release_id'=>'number'), 
